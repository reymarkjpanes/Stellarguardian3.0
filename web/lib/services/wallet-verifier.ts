 ...
 * and we verify the transaction signature.
 *
 * Works with the existing `wallet_challenges` table schema — no new columns needed.
 * The challenge transaction is deterministic: it can be rebuilt from (publicKey, nonce).
 */
import "server-only";
...
/**
 * Build a deterministic challenge transaction from a public key and nonce.
 * This can be called both during issuance and during verification — the
 * output is identical for the same inputs, removing the need to store XDR.
 */
 ...
 *
 * Returns a challenge transaction XDR for the wallet to sign.
 * The nonce is stored as bytea in the existing schema.
 */
export async function issueChallenge(
  ...
  const challengeXdr = buildChallengeTransaction(claimedPublicKey, nonceHex);

  // Store in existing schema — nonce as hex-encoded bytea
  // Supabase/PostgREST accepts hex strings with \\x prefix for bytea columns
 ...
 *
 * The client signs the challenge transaction with their wallet.
 * We rebuild the same TX from stored nonce + public key, then verify
 * that the signed XDR contains a valid signature from the claimed key.
 */
export async function verifyChallenge(
  ...
  // Parse the signed transaction and verify the signature
  let verified = false;
  try {
    const signedTx = TransactionBuilder.fromXDR(signedXdr, NETWORK_PASSPHRASE);
    const keypair = Keypair.fromPublicKey(publicKey);

    // The transaction hash is what the wallet actually signed
    ...
    throw new BadRequestError(
      "Signature verification failed. Invalid signed transaction format.",
  ...
  // Upsert wallet record as Verified
  const walletProvider = options?.provider ?? "Freighter";
  const walletNetwork =
    options?.networkMode ?? (process.env.STELLAR_NETWORK_MODE ?? "testnet");
  ...
  if (walletError) {
    throw new Error(
      `Failed to update wallet verification status: ${walletError.message}`,
    );
  }
