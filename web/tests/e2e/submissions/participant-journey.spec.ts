...
test.describe('Submission Hub - Participant Journey', () => {
  test.beforeEach(async ({ page, auth }) => {
    ...
    // await auth.login('captain@team-alpha.com');
    // For local testing, we bypass UI login or assume mock server:
    await page.goto('/teams/team-alpha/submission');
  });

  test('Captain can view the submission hub, edit draft, upload asset, and submit', async ({ page }) => {
    // 1. View Hub
    ...
    await page.fill('textarea[placeholder="Enter your pitch..."]', 'This is our amazing project pitch.');
