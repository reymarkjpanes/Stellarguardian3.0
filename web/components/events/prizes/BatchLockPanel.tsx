...
export function BatchLockPanel({ batchId, status, setStatus, categories, allocations }: any) {
  ...
  const handleLock = async () => {
    ...
    const confirmed = confirm('Are you sure? Locking will permanently freeze these allocations and hand them over to Escrow.');
    if (!confirmed) return;
  ...
  const totalBudget = categories.reduce((s: number, c: any) => s + Number(c.total_amount), 0);
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="card p-6 bg-background space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Review the overall totals before finalizing.</p>
        </div>
        ...
        <div className="flex gap-4 pt-4 border-t">
          ...
          {status === 'Validated' && (
            <Button onClick={handleLock} disabled={loading} variant="destructive" className="w-full">
              Lock & Send to Escrow
            </Button>
          )}
