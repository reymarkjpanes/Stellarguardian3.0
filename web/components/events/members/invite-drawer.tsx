...
export function InviteDrawer({ eventId, isOpen, onClose, onRefresh }: InviteDrawerProps) {
  ...
  async function handleCancelInvite(invitationId: string) {
     try {
       const res = await fetch(`/api/events/${eventId}/invitations/${invitationId}`, { method: "DELETE" });
       if (res.ok) {
          loadInvitations();
       }
     } catch(e) {}
      ...
      <div className="absolute inset-y-0 right-0 w-full max-w-md bg-[var(--bg)] shadow-2xl transition-transform duration-300 translate-x-0 border-l border-[var(--border)] overflow-y-auto flex flex-col">
        ...
        <div className="p-6 flex-1 space-y-8">
           <form onSubmit={handleInvite} className="space-y-4">
              <div>
                 <label className="block text-sm font-medium text-[var(--text)] mb-1">Invite by Email</label>
                 <input
                   type="email"
                   value={email}
           ...
           <div className="border-t border-[var(--border)] pt-6">
              ...
              {loadingInvites ? (
                 ...
                 <div className="space-y-3">
                    {invitations.map(inv => (
                       <div key={inv.id} className="flex items-center justify-between p-3 rounded-lg border border-[var(--card-border)] bg-[var(--card-bg)]">
                          <div>
