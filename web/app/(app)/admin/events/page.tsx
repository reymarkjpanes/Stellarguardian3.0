  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-[var(--text)]">Manage Events</h1>
            ...
            {(events ?? []).map((ev) => (
              <tr key={ev.id} className="border-b border-[var(--border)] last:border-0">
                <td className="px-4 py-3 font-medium">
                  <Link href={`/events/${ev.id}`} className="text-[var(--text)] hover:text-[var(--accent)]">
                    {ev.title}
                  </Link>
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-3">
                    <Link href={`/events/${ev.id}`} className="text-xs font-medium text-[var(--text)] hover:underline">
                      View
                    </Link>
                    {ev.state !== "Cancelled" && ev.state !== "Archived" && (
                      <form action={updateEventState}>
                        <input type="hidden" name="eventId" value={ev.id} />
                        <input type="hidden" name="newState" value="Cancelled" />
                        <button type="submit" className="text-xs font-medium text-[var(--destructive)] hover:underline">
                          Cancel
                        </button>
                      </form>
                    )}
                    {ev.state !== "Archived" && (
                      <form action={updateEventState}>
                        ...
                        <button type="submit" className="text-xs font-medium text-[var(--text-muted)] hover:underline">
                          Archive
                        </button>
                      </form>
                    )}
