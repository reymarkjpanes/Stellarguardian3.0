    ...
    >
      {activeTab === "overview" && (
        <div className="space-y-6">
          ...
          <section>
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Compatibility Score</h3>
            <div className="p-4 rounded-xl border bg-muted/30">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm mt-4">
                <div className="flex justify-between"><span>Skills</span> <span className="font-medium text-green-500">★★★★★</span></div>
                <div className="flex justify-between"><span>Timezone</span> <span className="font-medium text-green-500">★★★★★</span></div>
                <div className="flex justify-between"><span>Languages</span> <span className="font-medium text-yellow-500">★★★★☆</span></div>
                <div className="flex justify-between"><span>Experience</span> <span className="font-medium text-yellow-500">★★★☆☆</span></div>
              </div>
            </div>
          </section>
          
          <section>
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Current Team</h3>
            {member.teamId ? (
      ...
      {activeTab === "skills" && (
        <div className="space-y-6">
           <section>
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Top Skills</h3>
            <div className="flex flex-wrap gap-2">
              {member.skills?.map((s: any) => (
                <span key={s.id} className="px-2.5 py-1 rounded-md bg-secondary text-secondary-foreground text-sm font-medium">
                  {s.name} {s.experienceLevel && <span className="text-muted-foreground font-normal opacity-75">· {s.experienceLevel}</span>}
                </span>
              ))}
            </div>
          </section>
