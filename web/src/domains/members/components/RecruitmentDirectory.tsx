...
export function RecruitmentDirectory({
  teams,
  isLoading,
}: {
  teams: TeamListDTO[];
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {[1, 2, 3, 4, 5, 6].map((i) => (
          <div key={i} className="h-56 rounded-xl bg-muted animate-pulse"></div>
        ))}
      </div>
    );
  }
  ...
  if (!teams || teams.length === 0) {
    return (
      <EmptyState
        title="No recruiting teams."
        description="Create your own team or check back later."
        action={{ label: "Create Team", href: "#" }}
      />
    );
  }
          ...
          <div className="mt-5 pt-4 border-t flex justify-between items-center">
            <div className="flex -space-x-2">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-7 w-7 rounded-full bg-muted border-2 border-card flex items-center justify-center text-[10px]"
                >
                  U{i}
                </div>
              ))}
