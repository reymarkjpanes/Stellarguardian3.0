...
export function ProgressiveChecklist({ requirements, assets, onSave, onUpload }: ProgressiveChecklistProps) {
  ...
  const isComplete = (req: any, asset: any) => {
        ...
        return (
          <div 
            ...
            className={`border rounded-lg overflow-hidden transition-all duration-200 ${
              isExpanded ? "border-indigo-300 ring-1 ring-indigo-300 shadow-md" : "border-gray-200 shadow-sm"
            } bg-white`}
          >
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-colors ${
                  complete ? "bg-green-100 border-green-500 text-green-600" : "bg-gray-50 border-gray-300 text-gray-500"
                }`}>
                  {complete ? (
                ...
                <div className="text-left">
                  <h3 className={`text-lg font-semibold ${complete ? "text-gray-900" : "text-gray-700"}`}>
                    {req.name}
                    {req.is_required && <span className="ml-2 text-xs text-red-500 uppercase tracking-wide">Required</span>}
                  </h3>
